...
const Layout = ({ children}) => {
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center bg-[linear-gradient(to_bottom_right,_#241B1E_0%,_#2C2427_100%)]">
    <motion.div
      ...
      >
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          onClick={toggleMusic}
          className="fixed top-4 right-4 z-50 bg-white/80 backdrop-blur-sm p-3 rounded-full shadow-lg border border-100/50"
        >
          {isPlaying ? (
            <div className="relative">
              <PauseCircle className="w-6 h-6 text-darkText" />
              <span className="absolute -top-1 -right-1 w-2 h-2 bg-green-500 rounded-full animate-pulse" />
            </div>
          ) : (
            <PlayCircle className="w-6 h-6 text-darkText" />
          )}
        </motion.button>

        <main className="relative h-full w-full pb-[100px] bg-[#2B2326]">
          {children}
        </main>
        <BottomBar />
        {/* Music Info Toast */}
